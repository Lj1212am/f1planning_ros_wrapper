...
class mpc_config:
    ...
    Rk: list = field(
        default_factory=lambda: np.diag([0.01, 0.6])
    )  # input cost matrix, penalty for inputs - [accel, steering_speed]
    Rdk: list = field(
        default_factory=lambda: np.diag([0.01, 0.6])
    )  # input difference cost matrix, penalty for change of inputs - [accel, steering_speed]
    Qk: list = field(
        default_factory=lambda: np.diag([0.0, 80.0, 0.0, 10.0, 0.0, 0.0, 80.0])
...
class NMPCPlanner:
    ...
    def mpc_prob_init(self):
        ...
        self.P = self.opti.parameter(self.config.NXK + 2, self.config.TK + 1)
        ...
        def f(x, u, p):
            ...
            # self.config.DF = mu * self.config.M * 9.81 / 2.0
            # self.config.DR = mu * self.config.M * 9.81 / 2.0
            DF = mu * self.config.M * 9.81 / 2.0
            DR = mu * self.config.M * 9.81 / 2.0
        ...
        self.opti.subject_to(self.X[:, 0] == self.P[:-2, 0])
        ...
        for k in range(self.config.TK):
            ...
            con = self.U[:, k]
            p = self.P[:, k + 1]
            cost_fn = cost_fn + (st - p[:-2]).T @ Q @ (st - p[:-2]) + con.T @ R @ con
    ...
    def mpc_prob_solve(self, goal_state, current_state):
        ...
        self.opti.set_value(
            self.P,
            ca.horzcat(
                ca.vertcat(current_state_vec, 0.0, 0.0),
